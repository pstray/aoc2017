...
$network[0] = Intcode->new(@code);
$network[0]->config(step => 1,
		    on_input => sub {
			my($self,$input_p) = @_;
			my $input;
			if ($self->have_input) {
			    delete $idle{$self->{input}[0]};
		    ...
		    on_output => sub {
			my($self,$output) = @_;
			push @{$self->{__packet}}, $output;
			if (@{$self->{__packet}} == 3) {
			    my($target,$x,$y) = @{$self->{__packet}};
			    printf "  Send f=%d t=%d [%d,%d]\n", $self->{input}[0],
			      $target, $x, $y;
			    $self->{__packet} = [];

			    if ($target >= 0 && $target < 50) {
				$network[$target]->add_input($x,$y);
			    }
			    elsif ($target == 255) {
				$solution1 = $y;
				@nat = ($x,$y);
...
while (1) {
    my $c = $network[$ci];
    # printf "Stepping %d, %d\e[K\r", $time, $ci;
    $c->cont();
    last LOOP1 if defined $solution1;
    if (++$ci >= 50) {
	$time++;
	$ci = 0;
    }
...
while (1) {
    if (50 == keys %idle) {
	printf "NAT (@nat)\n";
	if ($last_nat_y eq $nat[1]) {
	    $solution2 = $last_nat_y;
	    last LOOP2;
	}
	$last_nat_y = $nat[1];
	$network[0]->add_input(@nat);
	delete $idle{0};
    }
    my $c = $network[$ci];
    # printf "Stepping %d, %d\e[K\r", $time, $ci;
